...
`,
        ...
        if (res.success) {
          ...
          const product = {
            ...
            goods_properties: JSON.stringify(info.param ? formatCategory(info.param) : []), //属性
...
async function gethistoryMessage(_, data) {
  ...
  if (isSuccess) {
    ...
    if (historyMessage) {
      ...
      console.log("主动拦截 历史记录", historyMessageString.history);
    ...
    console.log("historyMessageString=========>", historyMessageString);
...
// 根据视频号体验分指标类型统一格式化二级指标分数。
function formatWxExperienceMetricScore(curValue, valueType) {
  // 百分比型指标统一转成百分数，但不保留 % 符号。
  if (valueType === "percent") {
    return Number(((curValue || 0) * 100).toFixed(4));
  }

  // 时间型和普通数值型都直接返回原始数值；时间接口本身就是秒。
  return curValue ?? "";
}

// 将视频号总览接口返回转换成统一的一层分数结构。
function buildWxExperienceOverviewData(data) {
  return {
    // 一级分直接取总览接口当前值。
    consumerScore: data?.dsrScore?.curValue ?? "",
    productScore: data?.productScore?.curValue ?? "",
    logisticsScore: data?.deliverScore?.curValue ?? "",
    serviceScore: data?.serviceScore?.curValue ?? "",
  };
}

// 将视频号商品体验接口返回整理成系统内部结构；商品二级只有分数。
function buildWxProductMetrics(data) {
  return {
    // 品质退款率需要转成百分数。
    品质退款率: {
      score: formatWxExperienceMetricScore(data?.productQualityRatio_30d?.curValue, "percent"),
    },
    // 商品好评率需要转成百分数。
    商品好评率: {
      score: formatWxExperienceMetricScore(data?.proValidGoodEvalRatio?.curValue, "percent"),
    },
  };
}

// 将视频号物流体验接口返回整理成系统内部结构；这里仅保留页面展示的三个二级指标。
function buildWxLogisticsMetrics(data) {
  return {
    // 次日及时揽收率需要转成百分数。
    次日及时揽收率: {
      score: formatWxExperienceMetricScore(data?.collectInTimeRatio_30d?.curValue, "percent"),
    },
    // 线路配送达标率需要转成百分数。
    线路配送达标率: {
      score: formatWxExperienceMetricScore(data?.waybillArriveOntimeRatio_30d?.curValue, "percent"),
    },
    // 物流负向反馈率需要转成百分数。
    物流负向反馈率: {
      score: formatWxExperienceMetricScore(data?.waybillNegtiveOrdRatio_30d?.curValue, "percent"),
    },
  };
}

// 将视频号服务体验接口返回整理成系统内部结构；时间类指标统一保留秒。
function buildWxServiceMetrics(data) {
  return {
    平均回复时长: {
      score: formatWxExperienceMetricScore(data?.serviceReplyLength_30d?.curValue, "number"),
    },
    不回复率: {
      score: formatWxExperienceMetricScore(data?.serviceNoreplyRatio_30d?.curValue, "percent"),
    },
    售后首次操作时长: {
      score: formatWxExperienceMetricScore(
        data?.aftersaleFirstOperationTime_30d?.curValue,
        "number",
      ),
    },
    退款自主完结时长: {
      score: formatWxExperienceMetricScore(data?.onlyRefundDealSec_30d?.curValue, "number"),
    },
    退货退款自主完结时长: {
      score: formatWxExperienceMetricScore(data?.returnRefundDealSec_30d?.curValue, "number"),
    },
    商责纠纷率: {
      score: formatWxExperienceMetricScore(data?.serviceComplaintRatio_30d?.curValue, "percent"),
    },
  };
}

// 将视频号其他指标接口返回整理成系统内部结构。
function buildWxOtherMetrics(data) {
  return {
    售后满意度: {
      score: formatWxExperienceMetricScore(data?.afterSaleNpsGoodRatio?.curValue, "percent"),
    },
  };
}

// 抽取视频号体验分接口请求逻辑，避免总览和各维度接口重复写 fetch 配置。
async function fetchWxExperienceScore(scoreTypeList) {
...
// 请求视频号体验分总览与各二级维度接口，并只返回整理后的数据。
async function buildWxExperienceScoreData() {
  // 总览接口返回一级分。
  const overviewData = await fetchWxExperienceScore([11, 12, 13, 14, 1000]);
  // 商品体验详情接口返回商品二级指标。
  const productData = await fetchWxExperienceScore([70, 64, 1000]);
  // 物流体验详情接口返回物流二级指标。
  const logisticsData = await fetchWxExperienceScore([72, 73, 71, 25, 105, 74, 78, 1000]);
  // 服务体验详情接口返回服务二级指标。
  const serviceData = await fetchWxExperienceScore([75, 76, 77, 67, 68, 69, 1000]);
  // 其他指标接口返回售后满意度。
  const otherData = await fetchWxExperienceScore([63, 1000]);

  return {
    ...buildWxExperienceOverviewData(overviewData),
    productMetrics: buildWxProductMetrics(productData),
    logisticsMetrics: buildWxLogisticsMetrics(logisticsData),
    serviceMetrics: buildWxServiceMetrics(serviceData),
    otherMetrics: buildWxOtherMetrics(otherData),
  };
}

// 获取视频号店铺体验分；当前只负责请求和整理数据，不在这里发 IPC。
const getShopExperienceScore = async () => {
  try {
    const formattedData = await buildWxExperienceScoreData();
    console.log("获取视频号店铺体验分返回:", formattedData);
    return formattedData;
  } catch (error) {
    console.log("获取店铺体验分失败:", error);
    return null;
  }
};
